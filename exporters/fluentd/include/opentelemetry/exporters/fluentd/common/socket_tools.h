...
#ifdef _MSC_VER
#pragma warning(push)
// Disable: warning C4267: 'argument': conversion from `size_t` to `int`,
// possible loss of data.
//
// WinSock vs POSIX sockets use different definition of socket payload size,
// e.g. in definition of socket ::send 'len' argument:
// - WinSock: int len
// - Linux:   size_t len
// - BSD:     size_t len
//
// We keep C++ method signature identical and prefer `size_t`. It is expected
// that Windows client cann not physically attempt to send a buffer larger than
// a size of max int.
//
...
// Unfortunately libcurl defines a structure that should otherwise be normally
// defined by afunix.h . When that happens, we cannot build the sockets library
...
struct Socket {
  ...
  int recvfrom(_Out_cap_(size) void *buffer, size_t size, int flags,
...
#endif
    return static_cast<int>(::recvfrom(m_sock, reinterpret_cast<char *>(buffer),
                                       size, flags, clientAddr, &len));
  }
  ...
  template <typename T> int getsockopt(int level, int optname, T &optval) {
#ifdef _WIN32
    int optlen = sizeof(T);
...
/// Socket Reactor
/// </summary>
struct Reactor : protected net::Thread {
  /// <summary>
...
public:
  ...
  void addSocket(const Socket &socket, int flags) {
    ...
    if ((flags == State::Readable) && (m_sockets.size() == 0)) {
      ...
      m_streaming = false;
      LOG_TRACE("Reactor: Adding datagram socket 0x%x with flags 0x%x",
                static_cast<int>(socket), flags);
    ...
    if (m_streaming) {
      ...
      if (it == m_sockets.end()) {
        LOG_TRACE("Reactor: Adding socket 0x%x with flags 0x%x",
                  static_cast<int>(socket), flags);
#ifdef _WIN32
        ...
        LOG_TRACE("Reactor: Updating socket 0x%x with flags 0x%x",
                  static_cast<int>(socket), flags);
      }
  ...
  virtual void onThread() override {
    ...
    if (!m_streaming) {
      ...
      LOG_TRACE("Reactor: socket 0x%x receive loop started...",
    ...
    while (!shouldTerminate()) {
...
#ifdef _WIN32
      DWORD dwResult =
          ::WSAWaitForMultipleEvents(static_cast<DWORD>(m_events.size()),
      ...
      if (dwResult > WSA_WAIT_EVENT_0 + m_events.size()) {
        LOG_WARN("Reactor: stale event on closed socket dwResult=%d",
                 (int)dwResult);
        continue;
      }
...
#ifdef __linux__
      {
        ...
        int result = ::epoll_wait(m_epollFd, events,
                                  sizeof(events) / sizeof(events[0]), 500);
        if (result == 0)
          continue;
        if (result < 0) {
        ...
        assert(result >= 1 && static_cast<size_t>(result) <=
                                  sizeof(events) / sizeof(events[0]));

        LOCKGUARD(m_sockets_mutex);
        for (int i = 0; i < result; i++) {
          auto it =
              std::find(m_sockets.begin(), m_sockets.end(), events[i].data.fd);
          assert(it != m_sockets.end());
          Socket socket = it->socket;
          int flags = it->flags;

          LOG_TRACE(
              "Reactor: Handling socket 0x%x active flags 0x%x (armed 0x%x)",
          ...
          if ((flags & Closed) && (events[i].events & (EPOLLHUP | EPOLLERR))) {
            LOG_TRACE("Reactor: handling socket 0x%x onSocketClosed",
                      static_cast<int>(socket));
            m_callback.onSocketClosed(socket);
          }
        }
