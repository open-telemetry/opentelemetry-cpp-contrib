...
namespace logs
{
  ...
  {
       json_["severityText"] = opentelemetry::logs::SeverityNumToText[static_cast<int>(severity)];
  ...
  void Recordable::SetTraceId(opentelemetry::trace::TraceId trace_id) noexcept 
  {
   ...
   void Recordable::SetSpanId(opentelemetry::trace::SpanId span_id) noexcept 
   {
                              ...
                              const opentelemetry::common::AttributeValue &value) noexcept
    {
        ...
        opentelemetry::exporter::fluentd::common::PopulateAttribute(json_[FLUENT_FIELD_PROPERTIES], key, value);
    }
    ...
    void Recordable::SetTimestamp(opentelemetry::common::SystemTimestamp timestamp) noexcept
    {
        json_["Timestamp"] =
            ...
            std::chrono::duration_cast<std::chrono::nanoseconds>(timestamp.time_since_epoch()).count() % 1000000000);
    }
