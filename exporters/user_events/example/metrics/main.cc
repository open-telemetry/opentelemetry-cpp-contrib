...
namespace user_events_exporter = opentelemetry::exporter::user_events::metrics;
...
void initMetrics(const std::string &name, const std::string &account_name) {
  ...
  user_events_exporter::ExporterOptions options{};
  std::unique_ptr<metric_sdk::PushMetricExporter> exporter{
      new user_events_exporter::Exporter(options)};
