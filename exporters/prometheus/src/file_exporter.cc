...
#  define OPENTELEMETRY_PROMETHEUS_FILE_HAVE_EXCEPTIONS 1
#endif

#ifdef OPENTELEMETRY_PROMETHEUS_FILE_HAVE_EXCEPTIONS
#  include <exception>
#endif
...
#  ifdef _MSC_VER
#    define PROMETHEUS_FILE_SNPRINTF(buffer, bufsz, ...) \
  ...
  for (size_t i = 0; i < fmt.size() && ret < bufz && running; ++i)
  {
    ...
    switch (fmt[i])
    {
      ...
      case 'N': {
        ...
        auto res          = PROMETHEUS_FILE_SNPRINTF(&buff[ret], bufz - ret, "%llu",
...
public:
   ...
   *
   * @param data a collection of records to add to the metricsToCollect collection
   */
...

    {
      ...
      if (options_.flush_count > 0)
      {
        if (file_->left_flush_metrics <= translated.size())
        {
          ...
          file_->flushed_metric_family_count.store(
              file_->metric_family_count.load(std::memory_order_acquire),
  ...
  bool ForceFlush(std::chrono::microseconds timeout) noexcept
  {
    ...
    // Limit the wait interval to a maximum of 1 second to avoid excessively long waits. When wake
    // up notifications are missed.
    if (wait_interval > std::chrono::microseconds{1000000})
    {
      wait_interval = std::chrono::microseconds{1000000};
    }

    std::size_t current_wait_for_flush_count =
        file_->metric_family_count.load(std::memory_order_acquire);
    ...
    while (timeout >= std::chrono::microseconds::zero())
    {
      ...
      {
        if (file_->flushed_metric_family_count.load(std::memory_order_acquire) >=
...
private:
  void Initialize()
  {
    ...
    std::string file_pattern;
    {
      ...
      for (std::size_t i = 0; options_.file_size > 0 && i < options_.rotate_size; ++i)
      {
        ...
        // The file size is also 0 when the file does not exist.
...

    {
      ...
      for (std::size_t i = 0; i + 1 < file_pattern.size(); ++i)
      {
        ...
        {
          int checked =
              static_cast<int>(file_pattern[i + 1]);  // NOLINT(bugprone-signed-char-misuse)
  ...
  std::shared_ptr<std::ofstream> OpenLogFile(bool destroy_content)
  {
    ...
    if (file_path_size <= 0)
    {
      OTEL_INTERNAL_LOG_ERROR("[Prometheus File] Generate file path from pattern "
    ...
    if (!options_.alias_pattern.empty())
    {
      ...
      if (file_path_len <= 0)
      {
        OTEL_INTERNAL_LOG_ERROR("[Prometheus File] Generate alias file path from "
      ...
      if (res != 0)
      {
        OTEL_INTERNAL_LOG_ERROR("[Prometheus File] Link " << file_->file_path << " to "
                                                          << alias_file_path
                                                          << " failed, errno: " << res);
#  if !defined(__CYGWIN__) && defined(_WIN32)
        OTEL_INTERNAL_LOG_ERROR(
            ...
            << "https://docs.microsoft.com/en-us/windows/desktop/Debug/"
               ...
               "details");
#  else
        OTEL_INTERNAL_LOG_ERROR("[Prometheus File]     you can use strerror("
...
#  endif
  ...
  void SpawnBackgroundWorkThread()
  {
      ...
      file_->background_flush_thread.reset(new std::thread([concurrency_file, flush_interval]() {
        ...
        while (true)
        {
...

          {
            ...
            concurrency_file->flushed_metric_family_count.store(current_metric_family_count,
                                                                std::memory_order_release);
          }
    ...
    {
      OTEL_INTERNAL_LOG_ERROR(
          "[Prometheus File] SpawnBackgroundWorkThread got exception: " << e.what());
    }
    catch (...)
    {
      OTEL_INTERNAL_LOG_ERROR("[Prometheus File] SpawnBackgroundWorkThread got unknown exception");
    }
#endif
  }
...
PrometheusFileExporter::PrometheusFileExporter(const PrometheusFileExporterOptions &options)
    : options_(options),
      is_shutdown_(false),
      backend_{std::make_shared<PrometheusFileBackend>(options)}
{}

::opentelemetry::sdk::metrics::AggregationTemporality
    PrometheusFileExporter::GetAggregationTemporality(
...
::opentelemetry::sdk::common::ExportResult PrometheusFileExporter::Export(
...
bool PrometheusFileExporter::IsShutdown() const
{
  return is_shutdown_;
}
