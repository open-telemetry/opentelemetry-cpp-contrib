...
#define CLID(conn) conn.client.toString().c_str()
    ...
    if (socket.accept(csocket, caddr)) {
      ...
      if (bind_address.isUnixDomain) {
        ...
        std::copy(std::begin(bind_address.m_data_un.sun_path),
                  std::end(bind_address.m_data_un.sun_path),
                  std::begin(caddr.m_data_un.sun_path));
      };
#endif
  ...
  virtual void ReadStreamBuffer(Connection &conn_tcp) {
    ...
    size_t size = conn_tcp.socket.readall(
        const_cast<char *>(conn_tcp.request_buffer.data()),
        conn_tcp.request_buffer.size());
    if (size > 0) {
      ...
      LOG_ERROR("Server: [%s] failed to read client stream, errno=%d",
                CLID(conn_tcp), errno);
  ...
  virtual void ReadDatagramBuffer(Connection &conn_udp) {
    ...
    int size = conn_udp.socket.recvfrom(
        (void *)(conn_udp.request_buffer.data()), 0xffff, 0, conn_udp.client);
    if (size > 0) {
  ...
  /**
   * @brief Handle a timeslice of sending data back to client. If sending is
   * blocked, but not all data has been sent, then this function returns with an
   * indication that it needs to be called for the same connection again.
   *
  ...
  bool WriteResponseBuffer(Connection &conn) {
    if (conn.response_buffer.empty()) {
      LOG_TRACE("Server: [%s] response blocked, empty response buffer!",
                CLID(conn));
      return false;
    }
    ...
    if (optval == SOCK_DGRAM) {
      total_bytes_sent = conn.socket.sendto(
          conn.response_buffer.data(),
          static_cast<int>(conn.response_buffer.size()), 0, conn.client);
      LOG_TRACE("Server: [%s] datagram sent %zu bytes", CLID(conn),
                total_bytes_sent);
      return false;
    }
    ...
    LOG_TRACE("Server: [%s] response complete, total sent %zu bytes",
              CLID(conn), total_bytes_sent);
    return false;
  }
  ...
  void onConnectionClosed(Connection &conn) {
    ...
    if ((!conn.state.count(Connection::Idle)) &&
        (!conn.state.count(Connection::Closing))) {
