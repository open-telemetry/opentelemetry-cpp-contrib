...
ETWDataTransport::ETWDataTransport(const std::string &etw_provider)
{
    ...
    {
        LOG_ERROR("ETWDataTransport:: Failed to initialize the ETW provider.  Metrics will not be published");
        provider_handle_ = INVALID_HANDLE;
    }
}

bool ETWDataTransport::Send(MetricsEventType event_type,
                    const char *data, 
                    uint16_t length)
{
    if (provider_handle_ == INVALID_HANDLE){
        LOG_ERROR("ETWDataTransport:: ETW Provider Handle is not valid. Metrics is dropped");
        return false;
    }
    ...
    auto result = ::EventWrite(provider_handle_,
                                &evtDescriptor,
                                descriptorSize,
                                dataDescriptor);
    if (result != ERROR_SUCCESS) {
        LOG_ERROR("ETWDataTransport:: Failed to publish metric to ETW. Error: %d", result);
        return false;
    }
    return true;
}
