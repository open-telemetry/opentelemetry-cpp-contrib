...
/// <summary>
/// The UUID structure represents the portable cross-platform implementation of
/// a GUID (Globally Unique ID).
/// </summary>
/// <remarks>
/// UUIDs identify objects such as interfaces, manager entry-point vectors
/// (EPVs), and class objects. A UUID is a 128-bit value consisting of one group
/// of eight hexadecimal digits, followed by three groups of four hexadecimal
...
struct UUID {
  ...
  /// <summary>
  /// A constructor that creates a UUID object from a hyphenated string as
  /// defined by https://tools.ietf.org/html/rfc4122#page-4
  /// </summary>
  /// <param name="uuid_string">A hyphenated string that contains the UUID
  /// (curly braces optional).</param>
  UUID(const char *uuid_string) {
    ...
    if (
        ...
        (11 ==
         sscanf_s(str, "%08lX-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X", &p0,
                  &p1, &p2, &p3, &p4, &p5, &p6, &p7, &p8, &p9, &p10)) ||
        // Parse input without dashes
        (11 == sscanf_s(str, "%08lX%04X%04X%02X%02X%02X%02X%02X%02X%02X%02X",
  ...
  /// A value of <i>true</i> specifies the more natural human-readable
  /// order.<br> A value of <i>false</i> (the default) specifies the same order
  /// as the .NET GUID constructor.
  /// </param>
  ...
  /// <summary>
  /// A constructor that creates a UUID object from three integers and a byte
  /// array.
  /// </summary>
  /// <param name="d1">An integer that specifies the first eight hexadecimal
  /// digits of the UUID.</param> <param name="d2">An integer that specifies the
  /// first group of four hexadecimal digits.</param> <param name="d3">An
  /// integer that specifies the second group of four hexadecimal
  /// digits.</param> <param name="v">A reference to an array of eight bytes.
  /// The first two bytes contain the third group of four hexadecimal digits.
  /// The remaining six bytes contain the final 12 hexadecimal digits.</param>
  ...
  size_t Hash() const {
    // Compute individual hash values for Data1, Data2, Data3, and parts of
    // Data4
    size_t res = 17;
    res = res * 31 + Data1;
    res = res * 31 + Data2;
    res = res * 31 + Data3;
    res =
        res * 31 + (Data4[0] << 24 | Data4[1] << 16 | Data4[6] << 8 | Data4[7]);
    return res;
  }

  /// <summary>
  /// Tests to determine whether two UUID objects are equivalent (needed for
  /// maps).
  /// </summary>
...
/// <summary>
/// Declare UUIDComparer as the Comparer when using UUID as a key in a map or
/// set
/// </summary>
struct UUIDComparer : std::less<UUID> {
  inline size_t operator()(UUID const &key) const { return key.Hash(); }

  inline bool operator()(UUID const &lhs, UUID const &rhs) const {
    return lhs.Hash() < rhs.Hash();
  }
};

#ifdef _WIN32
static inline bool sha1(const BYTE *pData, DWORD nData, BYTE *pHashedData,
                        DWORD &nHashedData) {
  bool bRet = false;
...
static inline GUID GetProviderGuid(const char *providerName) {
  ...
  unsigned long a =
      (((((buffer2[3] << 8) + buffer2[2]) << 8) + buffer2[1]) << 8) +
      buffer2[0];
