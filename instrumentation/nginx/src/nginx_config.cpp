...
ngx_int_t OtelNgxConfHandler(ngx_conf_t* cf, ngx_int_t last) {
  ...
  for (ngx_uint_t i = 0; cf->cycle->modules[i]; i++) {
    ...
    for (/* void */; cmd->name.len; cmd++) {
      ...
      if (
        cf->cycle->modules[i]->type != NGX_CONF_MODULE &&
      ...
      if (!(cmd->type & NGX_CONF_BLOCK) && last != NGX_OK) {
        ngx_conf_log_error(
          NGX_LOG_EMERG, cf, 0, "directive \"%s\" is not terminated by \";\"", name->data);
        return NGX_ERROR;
      }

      if ((cmd->type & NGX_CONF_BLOCK) && last != NGX_CONF_BLOCK_START) {
        ngx_conf_log_error(
          NGX_LOG_EMERG, cf, 0, "directive \"%s\" has no opening \"{\"", name->data);
        return NGX_ERROR;
      }
...
invalid:
  ...
  ngx_conf_log_error(
    NGX_LOG_EMERG, cf, 0, "invalid number of arguments in \"%s\" directive", name->data);
